...
export const useCartStore = defineStore("cart", () => {
  ...
  watchEffect(() => {
    const subtotalA = items.value.reduce(
      (total, item) => total + item.quantity * item.price,
      0
    );
    taxes.value = Number((subtotalA * TAX_RATE).toFixed(2));
    ...
    try {
      ...
      items.value.forEach(async item => {
        ...
        await runTransaction(db, async (transaccion) => {
            const currentProduct = await transaccion.get(productRef)
            const availability = currentProduct.data().availability - item.quantity
            transaccion.update(productRef, {availability})
        })   
       })
